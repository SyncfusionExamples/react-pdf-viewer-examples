...
This sample demonstrates real-time collaborative editing in the Syncfusion React PDF Viewer. It uses the Syncfusion Collaborator client package with a Node.js Collaboration Server to synchronize PDF Viewer actions between users.
...
├── Client/    # React PDF Viewer application
...
The client is configured to use this URL in `Client/src/App.jsx` through the `SERVICE_URL` constant. If the server URL changes, update the client configuration to use the same URL.
...
Vite will display the local client URL, normally similar to:
...
3. Copy the room ID from the browser URL or from the collaboration status bar. The room ID is the `id` query parameter, for example:
   ...
   ```text
   http://localhost:5173/?id=3tlfqgkp854
   ```
...
- **The client cannot connect:** Confirm that the server is running on port `8081` and that `SERVICE_URL` in `Client/src/App.jsx` matches the server URL.
